...
export default {
  ...
  methods: {
    ...
    _updateStyle: function (newitem, doFilter = true) {
      ...
      if (doFilter) _arr = [...this.filterStyleArr(newitem)]
      else _arr = [...this.StyleArr]

      if (this.targetEl && this.StyleArr) {
        const TargetEL = this.cleanClassname(this.targetEl)
        console.log("_updateStyle", newitem, doFilter, _arr)
        this.cleanDOMTokenList(this.targetEl, [TargetEL, ..._arr])
      }
    },
